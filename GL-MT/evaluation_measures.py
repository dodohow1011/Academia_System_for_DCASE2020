...
from models.models import generate_label
...
def adaptive_median_filter(pred_strong, decoder):
    ...
    for class_id in range(10):
        ...
        pred_strong_m[:, class_id] = scipy.ndimage.filters.median_filter(pred_strong[:, class_id].reshape((628, -1)), (m, 1)).squeeze()
    ...
    for i, ((input_data, _), indexes) in enumerate(dataloader):
        ...
        for j, pred_strong_it in enumerate(pred_strong):
            for threshold in thresholds:
                ...
                pred_strong_it = adaptive_median_filter(pred_strong_it, decoder)
